...
const buttonGuardarCompra = document.getElementById('buttonGuardarCompra');
buttonGuardarCompra.addEventListener('click', function(e){
    e.preventDefault();

    if(ProductosCarrito != null){
        let Compras = JSON.parse(localStorage.getItem('Compras'))
        let NuevoId = Math.max(...Compras.map(producto => parseInt(producto.Id)))+1;
        let newCompra = {
            Id: NuevoId,
